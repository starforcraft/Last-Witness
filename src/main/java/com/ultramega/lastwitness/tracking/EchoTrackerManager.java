...
public final class EchoTrackerManager {
    ...
    public static void record(final MinecraftServer server, final LivingEntity entity) {
        ...
        activeTracker(store, entity).record(entity);
        store.setDirty();
    }
    ...
    public static void recordEntityEvent(final MinecraftServer server,
                                         ...
                                         final byte eventId) {
        final EchoTrackerSavedData store = getStore(server);
        activeTracker(store, entity).recordEntityEvent(entity, eventId);
        store.setDirty();
    }
    ...
    public static void recordDamageEvent(final MinecraftServer server,
        ...
        final EchoTrackerSavedData store = getStore(server);
        final EchoTracker replayTracker = activeTracker(store, replayedEntity);
    ...
    private static EchoTracker activeTracker(final EchoTrackerSavedData store, final LivingEntity entity) {
        return store.activeTrackers.computeIfAbsent(entity.getUUID(), ignored -> new EchoTracker());
    }

    private static void removeOutsideTrackers(final EchoTrackerSavedData store, final UUID replayTrackId) {
    ...
    private static EchoTrackerSavedData getStore(final MinecraftServer server) {
        return server.getDataStorage().computeIfAbsent(EchoTrackerSavedData.TYPE);
    }
}
